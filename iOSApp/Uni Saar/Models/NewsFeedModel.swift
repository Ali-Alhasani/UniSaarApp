...
final class NewsFeedModel: Sendable {
...
final class NewsModel: Sendable {
    ...
    init(json: [String: Any]) {
        ...
        if let happeningDate = jsonFromated["happeningDate"].string {
            annoucementDate = happeningDate
            isEvent = true
        } else {
            annoucementDate = jsonFromated["publishedDate"].stringValue
            isEvent = false
        }
        title = jsonFromated["title"].stringValue
        newsID = jsonFromated["id"].intValue
        subTitle = jsonFromated["description"].stringValue
        categoryName = jsonFromated["categories"].dictionaryObject as? [String: String] ?? [:]
        imageURLString = jsonFromated["imageURL"].stringValue
        newslink = jsonFromated["link"].stringValue
    }
}

final class NewsCategories: Sendable {
    let categoryID: Int
    let categoryName: String
    init(json: [String: Any]) {
        let jsonFromated = JSON(json)
        categoryID = jsonFromated["id"].intValue
        categoryName = jsonFromated["name"].stringValue
    }
}

extension NewsModel {
    nonisolated(unsafe) static let deomJSON: [String: Any] = ["date": "12/05/2019", "title": "Echt jetzt? – Eine öffentliche Vortragsreihe über die Realität", "id": 100,
                                                              "description": """
